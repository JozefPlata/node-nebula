...
That's it. Open the browser at `localhost:8080`
...
4. Some libraries have more dependencies than others. `react` with resolve to just one, but `express` to many more. Because resolving is done recursively, this might take some time for huge libraries (try `nuxt`). Currently only `dependencies` are resolved, `devDependencies` are ignored.
